...
Server: `https://mcp.exa.ai/mcp/oauth` (configured by this plugin).

OAuth is required: on first connection the client opens `auth.exa.ai`, the user signs in with Google / SSO / email, and the JWT is attached automatically. No key to copy.

On auth / rate-limit errors, surface the fix (re-authenticate via the client's MCP auth flow) — don't fall back to generic web search.
...
Use your agent/task tool to dispatch subagents. Reference file paths are relative to the directory this file was loaded from.

Use the fastest available model for subagents.
...
Then: Format output beautifully, filling up no more than one scroll length of the screen. Include hyperlinked text where relevant. Below it, you may also include things (in a short, easy-to-read format) that:
