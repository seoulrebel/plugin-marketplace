...
This plugin bundles the Firecrawl MCP server. When it's connected, prefer the native **`firecrawl_agent`** tool over the CLI (and **`firecrawl_agent_status`** to poll a running agent) — it needs no local install and authenticates through the plugin's one-time browser sign-in. The `firecrawl` CLI commands below are the fallback for when the MCP isn't connected.
